...
def main():
    ...
    while current_time < MAX_TIME:
        ...
        if high_index - low_index > 0 and high_index != 0:
            match_quality = np.exp(
                -1 * PICKY * (np.abs(candidate[SCORE] - np.array(match_scores[low_index:high_index]))))
            time_quality = np.exp(HASTE * (np.array(match_atime[low_index:high_index]) - current_time)) + 1
            quality = np.multiply(match_quality, time_quality)
            match_index = low_index + np.argmax(quality)
